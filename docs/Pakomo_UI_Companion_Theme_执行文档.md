...
| Home 是 `Column`,无 Box 叠层、无紧急按钮、无角色装饰 | `HomeScreen.kt:174` | 角色装饰 / 紧急按钮是**新增**,列为 Phase 3 + 待决策,见 §6。 |
...
val CompanionColors = PakomoColors(
    background = Color(0xFFF4F2F7),   // 暖白 / 淡灰紫
    surface = Color(0xFFFCFBFE),
    surfaceFold = Color(0xFFF1EEF6),
    border = Color(0xFFEAE6F1),
    textPrimary = Color(0xFF262330),  // 暖近黑
    textSecondary = Color(0xFF635E70),
    muted = Color(0xFFA39FB0),
    accent = Color(0xFF43618F),       // 灰蓝 / 偏海军
    accentStrong = Color(0xFF33507A),
    accentTint = Color(0xFFE6ECF5),
    statusRunning = Color(0xFF43618F),
    statusStopped = Color(0xFF9A96A5),
    statusIdle = Color(0xFFC79A54),   // 柔化的暖黄
    errorContainer = Color(0xFFF9EEF0),
    errorBorder = Color(0xFFE9CBD2),
    warning = Color(0xFF9A6A2E),
    warningContainer = Color(0xFFF6EAD6),
    scopeTrack = Color(0xFFEDEAF3),
    scopeDisabled = Color(0xFFE0DCE8),
    danger = Color(0xFFBA5A54),
    glitchCyan = Color(0xFF8FC8CF),   // 低饱和浅青
    glitchPink = Color(0xFFC7A7D6),   // 低饱和粉紫
    emergency = Color(0xFFE0897C),    // 柔和珊瑚
)
```

### 4.1 状态卡角色槽(`HomeScreen.kt` → `ServiceStatusCard`)
...
**文档 §18 的 High Latency / Packet Loss / Disconnected 不在引擎状态里**,处理方式二选一(列为 Phase 3):
- (推荐)从 `RuntimeStats`(`droppedTransfers`、`delayedTransfers` 等)**派生**出轻提示,不新增引擎状态;
- 或本期不做,仅保留 5 态。
...
## 8. 资源与占位策略(本期)
- 不引入图片加载库、不新增 webp。角色/状态图**全部用 Compose 占位**:`Box` 纯色块或简单 `Canvas`/矢量,按 §5 变色。
- 预留资源命名(将来替换,遵从项目现有 `res/drawable` 规范):`status_stopped / status_running / status_idle / status_error / empty_rules / empty_logs / about_character`。
- 真实美术接入时,只替换占位 Composable 内部实现,调用点不动。
...
6. **PR-3**(待决策):底部装饰 / 紧急按钮 / 派生故障 / 关于角色。
