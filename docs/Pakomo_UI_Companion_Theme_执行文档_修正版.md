...
**批次 A · 状态卡角色(已接线,优先)** —— 右侧局部裁切(脸+肩+托举数据方块那只手),**非全身**。

| 资源名 | 状态 | 表现(参考 §5) | 关键约束 |
|---|---|---|---|
| `mascot_status_running` | 运行中 | 托举发光青色数据方块,自信/微笑 | 见下"背景适配" |
| `mascot_status_idle` | 空闲运行 | 提示/指向(催促选目标) | 同上 |
| `mascot_status_starting`| 正在启动 | 观察/等待 | 同上 |
| `mascot_status_stopped` | 已停止 | 安静/休息(可略降透明度) | 同上 |
| `mascot_status_error` | 启动失败 | 困惑 + 一枚轻微错位碎块 | 同上 |

- **构图**:主体靠右,**左侧 ~55–60% 必须留空**(状态文字压在其上,不能被角色挡)。交付比例约 3:2 横向(如 312×208 @xxhdpi),右对齐。
- **背景适配(关键难点)**:状态卡底色随状态变**实色**(运行=蓝、空闲=橙、停止=灰、失败=浅底)。角色需带**描边/柔和轮廓光**,保证在深色实底与浅底上都读得出;避免大面积纯白(浅底消失)或大面积纯黑(深底消失)。
  - 若某些高饱和态(尤其运行蓝)实测压不住,退路:仅在低饱和态显示角色,高饱和态回落到占位数据方块。这点等你出图后实测再定。

**批次 B · 空状态角色(最好出、性价比最高;空状态区是浅底,无背景适配难题)** —— 半身/全身,居中构图。

| 资源名 | 场景 | 表现 |
|---|---|---|
| `empty_rules` | 无自定义规则 | Pako 抱着空数据盒 |
| `empty_logs` | 无日志输出 | Pako 看着空终端 |
| `error_dns` | 域名无法解析 | 数据方块碎成问号(可含/不含 Pako) |
| `empty_apps` *(可选)* | 无可选应用 | Pako 摊手 |

- 居中构图,交付约 1:1(如 480×480 @xxhdpi),浅底透明。

**批次 C · 品牌 / 关于(低优先)**

| 资源名 | 用途 | 约束 |
|---|---|---|
| `brand_mark` | 顶栏品牌图标 / App Icon 极简版(小黑兽发饰) | 小尺寸清晰,~72×72 |
| `about_pako` | 关于页较完整立绘 | 透明,居中 |
| `mascot_home` *(可选)* | 首页底部装饰(§5.1.6) | 右下角构图,透明 |

> 建议先出**批次 B**(最好画、立刻有角色感、无背景适配坑),再攻**批次 A**(需要背景适配打磨)。
