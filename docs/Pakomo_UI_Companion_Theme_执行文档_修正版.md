...
- 当前 App 和引擎不存在独立的“紧急恢复正常网络”能力，因此本执行方案**不新增**该按钮，也不把它列为 UI 验证项。
- 视觉文档中的“紧急恢复”仅是概念建议，不能据此创造新的功能承诺。
...
> **编号即占位标签**:App 里每个角色位现在都是"白底黑框黑字编号"占位框(如 `B1 empty_apps`),编号与下表一一对应,方便实机对照"哪个槽在哪"。真图按资源名给我即可。

**状态卡装饰(原批次 A,已改为纯绘制,无需出图)** —— 状态卡右侧**不再放角色形象**(一屏两处角色不够"轻")。改为 `StatusDecor`:一组随状态点亮/熄灭/错位的**抽象数据块**(`ui/components/PakomoArtwork.kt`,Compose 绘制,饱和卡底用白、浅底用青/红以保对比)。角色形象只集中在首页底部主卡 C1 一处。此项不产美术资源。
...
| 编号 | 资源名 | 用途 | 约束 |
|---|---|---|---|
| C1 | `mascot_home_{n}` | 首页**角色主卡**(铺满接管范围卡下方→App 底部整片区,**已接线占位**) | 统一一档差分,见下方案 |
| C2 | `brand_mark` | ~~顶栏品牌图标~~(顶栏图标已移除,**暂不需要**)/ 将来 App Icon 极简版可复用 | 暂缓 |
| C3 | `about_pako` | 关于页较完整立绘(关于页已存在,未接角色) | 透明、居中,~1:1 |

> **C1 方案(统一最小档尺寸 + 差分随机)**:主卡区高度随状态变(约 394–554dp),但**统一按最小档一个尺寸出图,不分档**。渲染:满宽、比例约 **0.95(宽÷高)**、**贴底**(`Alignment.BottomCenter` + `fillMaxWidth().aspectRatio(0.95f)`);区域更高时角色保持这个尺寸贴底、**上方自然留空**。一种尺寸通吃四种状态。
> - **差分**:同一尺寸/比例出 **3–5 张**(不同姿势/表情,做多点没事),命名 `mascot_home_1..n`;切状态时**随机取一张**。
> - 通用:透明背景、**主体贴底、横向满构图**、**左上留白**给规则/日志/设置三行导航文字避让、主体集中下部、不含文字。
>
> 已接线占位:B1–B3、C1(B 各一张一位;C1 真图同尺寸差分几张,随机)。状态卡装饰纯绘制不产图;B4/C2/C3 等对应功能做出来再补。
> 建议先出**批次 B**(最好画、无背景适配坑),再攻 **C1** 同尺寸差分。
